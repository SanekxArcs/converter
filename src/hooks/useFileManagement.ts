...
import { generateAutoMetadata } from "../utils/metadataUtils";
...
export const useFileManagement = () => {
  ...
  const validateAndProcessFiles = React.useCallback((files: File[]) => {
    ...
    if (files.length > supportedFiles.length) {
      setError(
        `${files.length - supportedFiles.length} unsupported files were ignored. Only PNG, AVIF, JPEG, and GIF files are supported.`,
      );
    }
      ...
      const newFiles: SelectedFile[] = await Promise.all(
        validFiles.map(async (file) => {
          try {
            ...
            const autoMetadata = generateAutoMetadata(file);
            return {
              id: generateUniqueId(),
              file,
